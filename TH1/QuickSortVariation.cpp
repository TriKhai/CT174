...
void QuickSortVariation(RecordType arr[], int start, int end) {
	int pivotIndex = FindPivot(arr, start, end);
	if(pivotIndex != -1) {
		int pivot = arr[pivotIndex].key;
		int pos = Partition(arr, start, end, pivot);
		QuickSortVariation(arr, start, pos-1);
		QuickSortVariation(arr, pos, end);
	}
}
...
int main(){
	...
	printf("Thuat toan Quick Sort Variation\n");
	printf("\nDu lieu truoc khi sap xep:\n");
	printSort(arr, size);
	printf("\nDu lieu sau khi sap xep:\n");
	QuickSortVariation(arr, 0, size-1);	
	printSort(arr, size);
